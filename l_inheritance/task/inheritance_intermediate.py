...
if __name__ == '__main__':
    ...
    while True:
        ...
        while True:
            ...
            # 내가만든 5번
            # elif menu_choice == 5:
            #     account_number = input(account_number_message)  # 계좌 번호 입력
            #     user = Bank.check_account_number(account_number)
            #     if user is not None:
            #         if user['password'] == input(password_message) and user['phone'] == input(phone_message):
            #             new_account_number = input(new_account_number_message)
            #             user['object'].account_number = new_account_number # 원래계좌 = 새로운 계좌로 변경
            #             print('재설정 완료')
            #         else:
            #             print(error_message)
            #     else:
